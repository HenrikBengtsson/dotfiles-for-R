...
#'
#' @imports prompt ps crayon grDevices profmem
startup_toolbox({
  last_value <- local({
    db <- list()
...
local({
  ## WORKAROUND:
  ## startup::startup() sources files with tryCatch(), which will cause
  ## the default prompt::set_prompt() to fail since it attempts to set
  ## a global calling handler
  oenv <- Sys.getenv("PROMPT_NO_GLOBAL", NA_character_)
  on.exit({
    if (is.na(oenv)) {
      Sys.unsetenv("PROMPT_NO_GLOBAL")
    } else {
      Sys.setenv("PROMPT_NO_GLOBAL" = oenv)
    }
  })
  Sys.setenv("PROMPT_NO_GLOBAL" = "true")
  ...
  prompt::set_prompt(local({
    ...
    mem <- function() {
      usage <- prompt::memory_usage()
#      usage <- sprintf("%s [%.0f%%]", usage$formatted, usage$percent)
    ...
    pkg <- function() {
      ## Using devtools?
      if (!"devtools_shims" %in% search()) return(NULL)
      blue(prompt:::devtools_package())  ## FIXME: Does not exist
    }
